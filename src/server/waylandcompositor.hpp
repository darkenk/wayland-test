...
class WaylandCompositor : public DestroyListener<WaylandSurface>,
        public WaylandGlobalObject<WaylandCompositor, wl_compositor_interface, struct wl_compositor_interface>
{
public:
    WaylandCompositor(wl_display* display, std::shared_ptr<X11Backend> output):
        WaylandGlobalObject(display) {
        mOutput = output;
...
protected:
    ...
    static const struct wl_compositor_interface* getInterface() {
        return &sInterface;
    }

private:
    static const struct wl_compositor_interface sInterface;
    friend WaylandGlobalObject<WaylandCompositor, wl_compositor_interface, struct wl_compositor_interface>;
