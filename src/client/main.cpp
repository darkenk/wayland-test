...
private:
    ...
    wl_buffer* createShmBuffer(wl_shm* shm, uint32_t width, uint32_t height) {
        ...
        wl_buffer* buff = wl_shm_pool_create_buffer(pool, 0, width, height, stride,
...
{
public:
    WaylandClient() : mDisplay(nullptr),
        mCompositor(nullptr), mSharedMemory(nullptr), mSurface(nullptr) {
